...
import (
	...
	"github.com/itda-skills/jindo/internal/updater"
	"github.com/spf13/cobra"
)

const (
	installScriptUnix    = "https://cdn.jsdelivr.net/gh/itda-skills/jindo@main/install.sh"
	installScriptWindows = "https://cdn.jsdelivr.net/gh/itda-skills/jindo@main/install.ps1"
)
...
var updateCmd = &cobra.Command{
	Use:     "update",
	Aliases: []string{"up"},
	Short:   "Update jd to the latest version",
	Long: `Check for and install updates to jd from GitHub releases.

Examples:
  jd update              # Check and update interactively
  jd update --check      # Check for updates only
  jd update -y           # Update without confirmation
  jd update --force      # Update without confirmation (same as -y)
  jd update -v v0.2.0    # Update to a specific version
  jd update --script     # Update using OS install script (curl/PowerShell)`,
	RunE: runUpdate,
}
...
func runUpdate(cmd *cobra.Command, args []string) error {
	...
	// Check only mode
	if updateCheck {
		fmt.Println("Update available! Run 'jd update' to install.")
		return nil
	}
...
func handleDevBuild() error {
	...
	fmt.Println("  go install github.com/itda-skills/jindo/cmd/jd@latest")
	fmt.Println()
...
func confirmUpdate(current, latest string) bool {
	fmt.Printf("Update jd %s → %s? [Y/n]: ", current, latest)
...
func runScriptUpdate() error {
	...
	case "windows":
		...
		if updateVersion != "" {
			script = fmt.Sprintf("$env:VERSION='%s'; ", updateVersion) + script
		}
		cmd = exec.Command("powershell", "-Command", script)
	default:
		script := fmt.Sprintf("curl -fsSL %s | bash", installScriptUnix)
		if updateVersion != "" {
			script = fmt.Sprintf("VERSION=%s %s", updateVersion, script)
		}
